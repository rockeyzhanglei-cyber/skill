...
> 核心：四类操作（加字段/加表/修改/删除）+ 值域修订，用统一的变更描述模板 `{表中文名}[{表英文名}]{操作}：…`（**`]` 后无空格**）；字段项写在 `[]` 内、半角逗号分隔，顺序 `[字段代码, 数据类型, 填报要求, 表示格式, …]`（仅代码必填；**类型项一律全大写**，如 `VARCHAR2(100)`、`NUMBER(18,3)`）。
> 本 Skill 采用其中的**详式**（字段项内附类型与填报要求，如 `[DAY_OP_FLAG,VARCHAR2(1),必填]`）；
...
| 变更清单 | 顶部 `/* */` 编号列出所有变更；同表加/删多字段用顿号合一行，如 `出院登记信息[INP_DISCHARGE]新增字段：日间手术病例标志[DAY_OP_FLAG,VARCHAR2(1),必填]、主管医生姓名[CHIEF_DOC_NAME,VARCHAR2(50),必填]` |
...
| 脚本块注释 | 与变更清单逐条一致；字段项用详细式 `表名中文[表名]新增字段：字段名中文[字段代码,数据类型,填报要求]`，类型项全大写（与 DDL 实际类型一致），同表加/删多字段顿号合一行、修改字段逐行 |
...
/*
新增表：表名中文[表名]
表名中文[表名]新增字段：字段名中文[字段代码,数据类型,填报要求]、字段名中文[字段代码,数据类型,填报要求]
