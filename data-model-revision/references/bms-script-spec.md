...
字段写在 `[]` 内，项与项之间用**半角逗号**分隔，顺序固定为（**v2 修订：类型提到填报要求之前**，张磊 2026-09-02 定稿）：

```
[字段代码, 数据类型, 填报要求, 表示格式, 定义, 值域, 约束, 默认值]
```

- 仅"字段代码"为必填，其余按数据模型实际有值才写；没有的项**省略，不写空位**。
- **数据类型项一律写全大写**：DDL 脚本写目标库映射后的物理类型（含长度/精度），如 `VARCHAR2(100)`、`NUMBER(18,3)`、`NUMERIC(18,3)`、`VARCHAR(400)`；数据标准侧若尚未映射，写标准数据类型标识，如 `S3`、`N1`。该项大小写不随脚本大小写格式变化，注释中的类型项恒为大写。
- 示例（字段代码 + 数据类型 + 填报要求）：`居民健康卡号[JMJKKH,VARCHAR2(100),必填]`。
...
| 操作 | 模板 | 示例 |
|------|------|------|
| 加字段 | `{表中文}[{表英文}]新增字段：{字段中文}[{字段项}]` | `出院登记信息[INP_DISCHARGE]新增字段：日间手术病例标志[DAY_OP_FLAG,VARCHAR2(1),必填]` |
...
```sql
/*
1. 出院登记信息[INP_DISCHARGE]新增字段：日间手术病例标志[DAY_OP_FLAG,VARCHAR2(1),必填]
...
**统一采用"详细式"字段项**（张磊定稿，唯一标准，不再提供简式/详式二选一）：字段项 `[]` 内按 `[字段代码, 数据类型, 填报要求, 表示格式, 定义, 值域, 约束, 默认值]` 顺序书写，**数据类型项全大写**，仅"字段代码"必填，其余有值才写、不写空位。

```sql
-- 标准写法（详细式，类型在前、填报要求在后，类型全大写）：
出院登记信息[INP_DISCHARGE]新增字段：日间手术病例标志[DAY_OP_FLAG,VARCHAR2(1),必填]
...
```sql
/*
1. 科室信息[BASE_DEPARTMENT]新增字段：记录状态[STATUS,VARCHAR2(1),必填]
2. 医护人员信息表[BASE_EMPLOYEE]新增字段：记录状态[STATUS,VARCHAR2(1),必填]
*/
```
...
- **语句级注释**：每条 `insert into edsm_revise_detail` 前用 `/* {变更描述} · {对象类型} */`，主句照抄清单，后缀标注对象类型（`元数据` / `数据集元素` / `值域` / `值域明细` / `数据集`）。
  ```sql
  /* 科室信息[BASE_DEPARTMENT]新增字段：记录状态[STATUS,VARCHAR2(1),必填] · 元数据 */
  /* 科室信息[BASE_DEPARTMENT]新增字段：记录状态[STATUS,VARCHAR2(1),必填] · 数据集元素 */
  ```
