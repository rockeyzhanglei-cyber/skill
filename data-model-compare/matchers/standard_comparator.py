...
#  - 诊断代码字段 kw='代码' 仅命中含"代码"的候选，西医出院诊断代码胜出
    ...
    # （P1-2 第二刀）P6 通道级常量已下沉至 matchers/auto_relation，
    # 此处以类属性别名保持 cls._XXX / self._XXX 的既有用法不变。
    _AUTO_REL_SYNONYMS = auto_relation.AUTO_REL_SYNONYMS
    _AUTO_REL_LOC_PREFIXES = auto_relation.AUTO_REL_LOC_PREFIXES
    _AUTO_REL_CARD_PREFIXES = auto_relation.AUTO_REL_CARD_PREFIXES
    _AUTO_REL_TAIL_KINDS = auto_relation.AUTO_REL_TAIL_KINDS
    _AUTO_REL_RESIDUE_MAP = auto_relation.AUTO_REL_RESIDUE_MAP
    _AUTO_REL_ADDR_LEVEL_KEYWORDS = auto_relation.AUTO_REL_ADDR_LEVEL_KEYWORDS
    _AUTO_REL_DISCRIMINATOR_MAP = auto_relation.AUTO_REL_DISCRIMINATOR_MAP
    _AUTO_REL_ATTR_TABLE_DISCS = auto_relation.AUTO_REL_ATTR_TABLE_DISCS
    _EXCLUSION_QUALIFIERS = auto_relation.EXCLUSION_QUALIFIERS

    @classmethod
    def _auto_rel_residue_match(cls, target_cn, source_cn):
        """判定实现已移至 matchers/auto_relation.residue_match（P1-2 第二刀拆分）。"""
        return auto_relation.residue_match(target_cn, source_cn)


    @classmethod
    def _resolve_discriminator_constraint(cls, target_cn, rel_table, auto_rel_discriminators):
        """判定实现已移至 matchers/auto_relation.resolve_discriminator_constraint（P1-2 第二刀拆分）。"""
        return auto_relation.resolve_discriminator_constraint(target_cn, rel_table, auto_rel_discriminators)


    @classmethod
    def _exclusion_qualifier_conflict(cls, cn, s_cn):
        """判定实现已移至 matchers/auto_relation.exclusion_qualifier_conflict（P1-2 第二刀拆分）。"""
        return auto_relation.exclusion_qualifier_conflict(cn, s_cn)
    ...
    @classmethod
    def _auto_rel_channel_synonym_hit(cls, name1, name2):
        """判定实现已移至 matchers/auto_relation.channel_synonym_hit（P1-2 第二刀拆分）。"""
        return auto_relation.channel_synonym_hit(name1, name2)


    @classmethod
    def _auto_relation_reuse_allowed(cls, prev_cn, new_cn):
        """判定实现已移至 matchers/auto_relation.reuse_allowed（P1-2 第二刀拆分）。"""
        return auto_relation.reuse_allowed(prev_cn, new_cn)
    ...
    @staticmethod
    def _paren_content_compatible(cn, s_cn):
        """判定实现已移至 matchers/auto_relation.paren_content_compatible（P1-2 第二刀拆分）。"""
        return auto_relation.paren_content_compatible(cn, s_cn)
    ...
    def _is_field_mapping_compatible(self, target_field, source_field, field_mapping):
        """判定实现已移至 matchers/field_compatibility.is_field_mapping_compatible（P1-2 拆分）。"""
        return field_compatibility.is_field_mapping_compatible(target_field, source_field, field_mapping)


    def _is_type_compatible_for_keyword(self, target_field, source_field):
        """判定实现已移至 matchers/field_compatibility.is_type_compatible_for_keyword（P1-2 拆分）。"""
        return field_compatibility.is_type_compatible_for_keyword(target_field, source_field)


    @staticmethod
    def _is_code_name_compatible(name1, name2):
        """判定实现已移至 matchers/field_compatibility.is_code_name_compatible（P1-2 拆分）。"""
        return field_compatibility.is_code_name_compatible(name1, name2)
    ...
    @staticmethod
    def _is_role_compatible_for_keyword(name1, name2):
        """判定实现已移至 matchers/field_compatibility.is_role_compatible_for_keyword（P1-2 拆分）。"""
        return field_compatibility.is_role_compatible_for_keyword(name1, name2)
    ...
    # （P1-2 拆分）字段"种类"常量已下沉至 matchers/field_compatibility，
    # 此处以类属性别名保持 self._FIELD_KIND_X 的既有用法不变，避免重复定义。
    _FIELD_KIND_NAME = field_compatibility.FIELD_KIND_NAME
    _FIELD_KIND_CODE = field_compatibility.FIELD_KIND_CODE
    _FIELD_KIND_SERIAL = field_compatibility.FIELD_KIND_SERIAL
    _FIELD_KIND_IDENT = field_compatibility.FIELD_KIND_IDENT
    _FIELD_KIND_SIGN = field_compatibility.FIELD_KIND_SIGN
    _FIELD_KIND_QUAL = field_compatibility.FIELD_KIND_QUAL
    _FIELD_KIND_ADDR = field_compatibility.FIELD_KIND_ADDR
    _FIELD_KIND_ATTR = field_compatibility.FIELD_KIND_ATTR

    @staticmethod
    def _field_kind_of(name):
        """判定实现已移至 matchers/field_compatibility.field_kind_of（P1-2 拆分）。"""
        return field_compatibility.field_kind_of(name)


    @staticmethod
    def _field_kind_compatible(name1, name2):
        """判定实现已移至 matchers/field_compatibility.field_kind_compatible（P1-2 拆分）。"""
        return field_compatibility.field_kind_compatible(name1, name2)


    # 复合字段名（"主表-子表xxx"）的语义主体取最后一段：
    # "手术记录-麻醉用药记录唯一标识" 的主体是"麻醉用药记录"，不是"手术记录"。
    @staticmethod
    def _composite_subject(name):
        """判定实现已移至 matchers/field_compatibility.composite_subject（P1-2 拆分）。"""
        return field_compatibility.composite_subject(name)
    ...
    @staticmethod
    def _is_role_compatible_for_synonym(name1, name2):
        """判定实现已移至 matchers/field_compatibility.is_role_compatible_for_synonym（P1-2 拆分）。"""
        return field_compatibility.is_role_compatible_for_synonym(name1, name2)


    @staticmethod
    def _is_concept_compatible_for_synonym(name1, name2):
        """判定实现已移至 matchers/field_compatibility.is_concept_compatible_for_synonym（P1-2 拆分）。"""
        return field_compatibility.is_concept_compatible_for_synonym(name1, name2)
